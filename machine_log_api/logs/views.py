...
@api_view(['GET'])
def get_operator_efficiency(request):
    """
    View to calculate and return the efficiency of operators.
    
    Efficiency is calculated as the ratio of time spent in production modes
    to the total time spent in all modes.
    
    Returns:
        JSON response with operator efficiency data
    """
    production_modes = [1]  # Production mode
    non_production_modes = [2, 3, 4, 5]  # Non-production modes

    # Aggregate time spent in each mode for each operator
    logs = MachineLog.objects.values('operator_id').annotate(
        production_time=Sum(
            ExpressionWrapper(
                F('end_time') - F('start_time'),
                output_field=DurationField()
            ),
            filter=Q(mode__in=production_modes)
        ),
        non_production_time=Sum(
            ExpressionWrapper(
                F('end_time') - F('start_time'),
                output_field=DurationField()
            ),
            filter=Q(mode__in=non_production_modes)
        )
    )
    ...
    efficiency_data = []
    for log in logs:
        total_time = log['production_time'] + log['non_production_time']
        efficiency = (log['production_time'] / total_time) * 100 if total_time else 0
        efficiency_data.append({
            'operator_id': log['operator_id'],
            'production_time': log['production_time'],
            'non_production_time': log['non_production_time'],
            'efficiency': efficiency
        })

    return Response(efficiency_data, status=200)
